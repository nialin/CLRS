...
static int rf_1, rf_2, rf_3, rf_4;

void hash_func_init(void);
int hash_func_universal(int key, int i);
...
void hash_func_init(void)
{
	...
	rf_3 = rand() % LARGE_PRIME;
	rf_4 = rand() % (LARGE_PRIME - 1) + 1;
}

int hash_func_universal(int key, int i)
{
	return (rf_1 * key + rf_2) + i * (rf_3 * key + rf_4) % LARGE_PRIME;
}
...
status hash_table_insert(struct hash_table *table, int key)
{
	...
	while(HASH_SLOT_USED == table->slots[pos = hash_func_universal(key, ++i) % HASH_TABLE_SIZE].flg && i < 1024)
		;
...
status hash_table_delete(struct hash_table *table, int key)
{
	...
	while(HASH_SLOT_DELETE == table->slots[pos = hash_func_universal(key, ++i) % HASH_TABLE_SIZE].flg ||
